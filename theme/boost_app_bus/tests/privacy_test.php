...
/**
 * Privacy tests for theme_boost_app_bus.
 *
...
use \theme_boost_app_bus\privacy\provider;

/**
 * Unit tests for theme_boost_app_bus/classes/privacy/policy
 *
 * @copyright  2018 Adrian Greeve <adriangreeve.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_boost_app_bus_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_preferences() {
        ...
        $exportedpreferences = $writer->get_user_preferences('theme_boost_app_bus');
        $this->assertCount(1, (array) $exportedpreferences);
        $this->assertEquals('false', $exportedpreferences->{provider::DRAWER_OPEN_NAV}->value);
        $this->assertEquals(get_string('privacy:drawernavclosed', 'theme_boost_app_bus'),
        ...
        $this->assertEquals(get_string('privacy:drawernavopen', 'theme_boost_app_bus'),
                $exportedpreferences->{provider::DRAWER_OPEN_NAV}->description);
    }
}
