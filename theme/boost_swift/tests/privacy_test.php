...
/**
 * Privacy tests for theme_boost_swift.
 *
...
use \theme_boost_swift\privacy\provider;

/**
 * Unit tests for theme_boost_swift/classes/privacy/policy
 *
 * @copyright  2018 Adrian Greeve <adriangreeve.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_boost_swift_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_preferences() {
        ...
        $exportedpreferences = $writer->get_user_preferences('theme_boost_swift');
        $this->assertCount(1, (array) $exportedpreferences);
        $this->assertEquals('false', $exportedpreferences->{provider::DRAWER_OPEN_NAV}->value);
        $this->assertEquals(get_string('privacy:drawernavclosed', 'theme_boost_swift'),
        ...
        $this->assertEquals(get_string('privacy:drawernavopen', 'theme_boost_swift'),
                $exportedpreferences->{provider::DRAWER_OPEN_NAV}->description);
    }
}
